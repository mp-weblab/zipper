    ...
    function zipper_create_zip_for_plugin($plugin_file)
    {
        ...
        if (is_dir($real_plugin_dir)) {
            ...
            foreach ($files as $file) {
                ...
                $relativePath = str_replace($real_plugin_dir . '/', '', $filePath);
        ...
        if ($zip->close() !== true) {
            return new WP_Error('zip_close', 'Erreur lors de la fermeture du ZIP');
        }

        clearstatcache();

        if (!file_exists($zip_path) || filesize($zip_path) < 200) {
            return new WP_Error('zip_failed', 'ZIP corrompu ou incomplet');
        }
